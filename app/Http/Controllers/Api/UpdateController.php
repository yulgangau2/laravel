    ...
    public function upload_layoff(Request $request)
    {
        ...
        try {
            if ($file) {
                ...
                $open = fopen('../storage/app/public/' . basename($path), "r");
//
                $data = [];
...
//        dd( $row,\Carbon\Carbon::createFromFormat('d/m/y',$row[7])->format('d/m/Y'));
                    $tmp = [
                        ...
                        'no' => $row[2],
                        'position' => $row[3],
                        'contain' => \Carbon\Carbon::createFromFormat('d/m/y',$contain)->addYears(543)->format('d/m/Y'),
...
//                    dd($row,$tmp);
                ...
                fclose($open);
                unlink('../storage/app/public/' . basename($path));
            ...
            try {
                ...
                if (isset($json['educationList']) && count($json['educationList']) > 0) {
                    ...
                    foreach ($full_educations as $idx => $education) {
                        ...
                        if ( $now_year ==$education['graduateYear']){
...
//                            ->first();
                    ...
                    foreach ($json['executiveList'] as $j => $excutive) {
                        ...
                        ///////////////////////////////////////////////////
//                        $year = (int)Carbon::createFromFormat('d/m/Y',$assign)->format('Y');
//                        $nowYear = (int)Carbon::now()->addYears(543)->format('Y');
//                        if ($year == $nowYear){
//
//                        }
...
//                    $user = Employee::query()->where('personalID', $json['personalID'])->first();
                    ...
                    if (isset($json['fameList'])) {
                        $fameList = $json['fameList'];


                        foreach ($fameList as $fame) {
                            $empFame = EmployeeFame::query()
                                ->where('receiveYear', (int)$fame['receiveYear'])
                                ->where('referenceDocument', $fame['referenceDocument'])
                                ->where('positionName', $fame['positionName'])
                                ->where('fameNameTha', $fame['fameNameTha'])
                                ->where('classAllType', $fame['classAllType'])
                                ->where('receiveReturnStatusName', $fame['receiveReturnStatusName'])
                                ->where('employee_id', $emp->id)
                                ->first();

                            if (!$empFame) {
                                $empFame = new EmployeeFame();
                                $empFame->receiveYear = (int)$fame['receiveYear'];
                                $empFame->referenceDocument = $fame['referenceDocument'];
                                $empFame->positionName = $fame['positionName'];
                                $empFame->fameNameTha = $fame['fameNameTha'];
                                $empFame->classAllType = $fame['classAllType'];
                                $empFame->receiveReturnStatusName = $fame['receiveReturnStatusName'];
                                $empFame->employee_id = $emp->id;
                                $empFame->save();
                            }
                        }
                    }
...
//        foreach ($empoyees as $i => $emp) {
            ...
            if ($perId) {
                ...
                try {
                    ...
                    if (isset($json['data']['AddressList'])) {
                        foreach ($json['data']['AddressList'] as $index => $address) {
                            $empAdd = EmployeeAddress::query()
                                ->where('AddressTypeNameTha', $address['AddressTypeNameTha'])
                                ->where('AddressName', $address['AddressName'])
                                ->where('employee_id',  $emp->id)
                                ->first();

                            if (!$empAdd) {
                                $empAdd = new EmployeeAddress();
                                $empAdd->AddressTypeNameTha = $address['AddressTypeNameTha'];
                                $empAdd->AddressName = $address['AddressName'];
                                $empAdd->employee_id = $emp->id;
                                $empAdd->save();
